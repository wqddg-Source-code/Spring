...
/**
 * Simple interface for bean definition readers that specifies load methods with  用于bean定义读取器的简单接口，该接口使用
 * {@link Resource} and {@link String} location parameters.
 *
 * <p>Concrete bean definition readers can of course add additional
 * load and register methods for bean definitions, specific to
 * their bean definition format.  具体的bean定义读者当然可以为bean定义添加额外的load和register方法，具体到他们的bean定义格式。
 *
 * <p>Note that a bean definition reader does not have to implement
 * this interface. It only serves as a suggestion for bean definition
 * readers that want to follow standard naming conventions.  请注意，bean定义读取器不必实现此接口。它仅作为希望遵循标准命名约定的bean定义读者的建议。
 *
