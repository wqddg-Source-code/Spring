...
/**
 * The root interface for accessing a Spring bean container.  Spring容器的根接口  没有这个就没有Spring
 *
 * <p>This is the basic client view of a bean container;
 * further interfaces such as {@link ListableBeanFactory   迭代出我的类信息   列表} and
 * {@link org.springframework.beans.factory.config.ConfigurableBeanFactory  父子关系  Sping 是父  SpringMVC是子  父不能访问子  子不能访问父}
 ...
 * 该接口由包含多个bean定义的对象实现，每个bean定义由一个字符串名唯一标识。
 * 根据bean定义，工厂将返回包含对象的独立实例（原型设计模式）或单个共享实例（单例设计模式的高级替代方案，其中实例是工厂范围内的单例）。
 * 将返回哪种类型的实例取决于bean工厂配置：API是相同的。自Spring 2.0以来，根据具体的应用程序上下文（例如web环境中的“请求”和“会话”范围），还可以使用其他范围。
 *
 ...
 * 这种方法的要点是BeanFactory是应用程序组件的中心注册表，并集中了应用程序组件配置（例如，单个对象不再需要读取属性文件）。
 ...
 *请注意，通常最好依靠依赖注入（“推”配置）通过setter或构造函数配置应用程序对象，
 * 而不是使用任何形式的“拉”配置，如BeanFactory查找。Spring的依赖注入功能是使用这个BeanFactory接口及其子接口实现的。
 *
 ...
 * 然而，实现可以直接在Java代码中返回它根据需要创建的Java对象。
 * 对于定义的存储方式没有限制：LDAP、RDBMS、XML、属性文件等。鼓励实现支持bean之间的引用（依赖注入）。
 *
 ...
 *与｛@link ListableBeanFactory｝中的方法不同，此接口中的所有操作也将检查父工厂，如果这是｛@LinkHierarchicalBean工厂｝。
 * 如果在此工厂实例中未找到bean，则将询问直接父工厂。这个工厂实例中的bean应该覆盖任何父工厂中同名的bean。
 *
